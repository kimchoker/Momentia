  ...
  return isMobile ? (
    ...
    <div className="fixed bottom-0 left-0 z-20 w-full bg-white shadow-lg flex justify-between items-center p-4">
      <Button
        ...
        className="flex flex-col items-center text-[#414868]"
      >
    ...
    >
      <div className="relative h-full flex flex-col px-3 py-4 overflow-y-auto shadow-md dark:shadow-zinc-800 border-none overflow-x-hidden justify-start">
        ...
        <div className="absolute top-[10px] -right-[0px] z-20">
          ...
          >
            <ChevronLeft
              className={`h-4 w-4 text-[#414868] transition-transform ease-in-out duration-700 ${!isOpen ? 'rotate-180' : 'rotate-0'}`}
            />
          </Button>
        </div>
        ...
        <Button
          variant="ghost"
          onClick={handleNewPost}
          className={`${isOpen ? 'w-[248px]' : 'w-[50px]'} justify-start h-10 mb-3 text-[#414868]`}
        >
          <span className={`${isOpen === false ? '' : 'mr-4'}`}>
            <SquarePen size={18} className="text-[#414868]" />
          </span>
          <p
            className={`max-w-[200px] truncate text-[#414868] ${!isOpen ? '-translate-x-96 opacity-0' : 'translate-x-0 opacity-100'}`}
          >
            글쓰기
          </p>
        </Button>
        ...
        <Button
          onClick={handleLogin}
          className={`${isOpen ? 'w-[248px]' : 'w-[50px]'} justify-start h-10 mt-5 fixed bottom-5 text-[#414868]`}
          variant="ghost"
        >
          <span className={`${isOpen ? 'mr-4' : ''}`}>
            {isLoggedIn ? (
              <LogOut size={18} className="text-[#414868]" />
            ) : (
              <LogIn size={18} className="text-[#414868]" />
            )}
          </span>
          <p
            className={`whitespace-nowrap text-[#414868] ${isOpen ? 'opacity-100' : 'opacity-0 hidden'}`}
          >
            {isLoggedIn ? '로그아웃' : '로그인'}
          </p>
        </Button>
