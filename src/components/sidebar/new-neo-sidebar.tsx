...
import { Swords, SquarePen, Users, Settings, MessageSquare, Bell, ChevronLeft, LogIn, LogOut, Home } from "lucide-react";
...
const Sibar = () => {
  ...
  return isMobile ? (
    ...
    <div className="fixed bottom-0 left-0 z-20 w-full bg-white shadow-lg flex justify-between items-center p-4">
      <Button variant="ghost" onClick={() => router.push("/")} className="flex flex-col items-center">
        <Home size={24} />
      </Button>

      <Button variant="ghost" onClick={handleNotification} className="flex flex-col items-center">
        <Bell size={24} />
      </Button>

      <Button variant="ghost" onClick={handleProfile} className="flex flex-col items-center">
        <Users size={24} />
      </Button>

      <Button variant="ghost" onClick={openModal} className="flex flex-col items-center">
        <Settings size={24} />
      </Button>
      ...
      <Button
        ...
        className="fixed right-5 bottom-24 bg-black text-white rounded-full w-14 h-14 p-3"
        variant="ghost"
      >
        <SquarePen size={24} />
      </Button>
      ...
      <div className="relative h-full flex flex-col px-3 py-4 overflow-y-auto shadow-md dark:shadow-zinc-800 border-none overflow-x-hidden ">
        ...
        <div className="absolute top-[10px] -right-[0px] z-20">
          <Button
            ...
            size="icon"
          >
            <ChevronLeft className={`h-4 w-4 transition-transform ease-in-out duration-700 ${!isOpen ? "rotate-180" : "rotate-0"}`} />
          </Button>
        </div>
        ...
        <Button variant="ghost" onClick={handleNewPost} className={`${isOpen ? "w-[248px]" : "w-[50px]"} justify-start h-10 mb-3`}>
          <span className={`${isOpen === false ? "" : "mr-4"}`}>
            <SquarePen size={18} />
          </span>
          <p className={`max-w-[200px] truncate ${!isOpen ? "-translate-x-96 opacity-0" : "translate-x-0 opacity-100"}`}>글쓰기</p>
        </Button>

        <Button variant="ghost" onClick={openModal} className={`${isOpen ? "w-[248px]" : "w-[50px]"} justify-start h-10 mb-3`}>
          <span className={`${isOpen === false ? "" : "mr-4"}`}>
            <MessageSquare size={18} />
          </span>
          <p className={`max-w-[200px] truncate ${!isOpen ? "-translate-x-96 opacity-0" : "translate-x-0 opacity-100"}`}>메시지</p>
        </Button>

        <Button variant="ghost" onClick={handleNotification} className={`${isOpen ? "w-[248px]" : "w-[50px]"} justify-start h-10 mb-3`}>
          <span className={`${isOpen === false ? "" : "mr-4"}`}>
            <Bell size={18} />
          </span>
          <p className={`max-w-[200px] truncate ${!isOpen ? "-translate-x-96 opacity-0" : "translate-x-0 opacity-100"}`}>알림</p>
        </Button>

        <Button variant="ghost" onClick={handleProfile} className={`${isOpen ? "w-[248px]" : "w-[50px]"} justify-start h-10 mb-3`}>
          <span className={`${isOpen === false ? "" : "mr-4"}`}>
            <Users size={18} />
          </span>
          <p className={`max-w-[200px] truncate ${!isOpen ? "-translate-x-96 opacity-0" : "translate-x-0 opacity-100"}`}>프로필</p>
        </Button>
        ...
        <Button variant="ghost" onClick={openModal} className={`${isOpen ? "w-[248px]" : "w-[50px]"} justify-start h-10 mb-3`}>
          ...
          <p className={`max-w-[200px] truncate ${!isOpen ? "-translate-x-96 opacity-0" : "translate-x-0 opacity-100"}`}>계정 설정</p>
        </Button>

        <Button onClick={handleLogin} className={`${isOpen ? "w-[248px]" : "w-[50px]"} justify-start h-10 mt-5 fixed bottom-5`} variant="ghost">
          <span className={`${isOpen ? "mr-4" : ""}`}>{isLoggedIn ? <LogOut size={18} /> : <LogIn size={18} />}</span>
          <p className={`whitespace-nowrap ${isOpen ? "opacity-100" : "opacity-0 hidden"}`}>{isLoggedIn ? "로그아웃" : "로그인"}</p>
        </Button>
      </div>
    </aside>
  );
